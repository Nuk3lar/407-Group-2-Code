%% Parameters
energy = 160;       % kV for the CSV files
nFilters = 9;      % number of filters
...
plotHandles = gobjects(nFilters,1);   % store plot handles for legend
legendLabels = strings(nFilters,1);   % store legend labels
x_scaled = [];                         % placeholder for X-axis

%% Loop through CSV files
for i = 1:nFilters
    % Build file name dynamically
    filename = sprintf('XStrahl Data//%dkvP_Filter_%d_2.csv', energy, i);
    ...
    legendLabels(i) = sprintf('Filter %d', i);
end
...
ylabel('$\mathrm{Planar\ fluence}\;\mathrm{[Counts/(incident\ particle\cdot cm^{2}\cdot keV)]}$', ...
       'Interpreter','latex');
title(sprintf('%dkV Planar Fluence for All Filters', energy));
...
filename = sprintf('beam_spectrum_%dkvP_filters.png', energy);
